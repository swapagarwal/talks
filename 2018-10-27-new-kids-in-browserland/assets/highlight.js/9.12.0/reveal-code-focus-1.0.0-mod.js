...
;(function(window, Reveal, hljs) {
  ...
  revealCodeStyle = document.querySelector('meta[data-code-style="reveal"]')
  var codeHideOpacity = revealCodeStyle ? 0.0 : 0.2;
  var codeResetOpacity = revealCodeStyle ? 0.0 : 1.0;
  ...
  function updateCodeOpacity() {
    ...
    if(linesInFocus.length == 0) {
        ...
        forEach(linesOfCode, function(line) {
          line.style.opacity = codeResetOpacity;
        });
    } else {
        // Some linesInFocus, reduce opacity on all linesNotInFocus.
        forEach(linesOfCode, function(line) {
            if(line.classList.contains('focus')) {
                line.style.opacity = codeShowOpacity;
            } else {
                line.style.opacity = codeHideOpacity;
            }
        });
    }
